...
def train_CEDA(model, device, train_loader, optimizer, epoch, lam=1., verbose=100):
    criterion = nn.NLLLoss()
    model.train()
...
def train_CEDA_gmm(model, device, train_loader, optimizer, epoch, lam=1., verbose=100):
    criterion = nn.NLLLoss()
    model.train()
